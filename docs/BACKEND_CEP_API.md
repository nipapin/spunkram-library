...
**Лимит устройств (по умолчанию 3, `CEP_DEVICE_LIMIT`, отдельно на каждый `client`):** если у этого клиента уже max активных устройств и входящая панель **не** совпала с занятым слотом, poll возвращает:
