...
2. SDK does **not** decode AtomX crypto; JS resolve/decrypt then pass plaintext paths (see Операция «Отречение»).
3. `FULL_PROJECT` + native `Motionflow.dll` is **unsupported** until licensed binaries ship.
