...
import { ensureDir, getCdnBaseManifestPath, getLocalStatePath, getStylePackageDir, getStylesDir, getStylesRoot, getUserControlsPath, getUserStyleDir, } from "./paths";
...
const isCepFs = () => typeof window !== "undefined" && typeof window.cep !== "undefined";
const readJson = (filePath) => {
    if (!isCepFs() || !fs.existsSync?.(filePath))
        return null;
    try {
...
const readMemoryState = () => {
    try {
        ...
        if (parsed.version !== LOCAL_STATE_VERSION)
            return emptyState();
        return {
            version: LOCAL_STATE_VERSION,
...
const writeMemoryState = (state) => {
    localStorage.setItem(MEMORY_KEY, JSON.stringify(state));
};
const readMemoryPackages = () => {
    try {
...
const writeMemoryPackages = (packages) => {
...
export const loadLocalState = () => {
    ...
    if (!fromDisk || fromDisk.version !== LOCAL_STATE_VERSION)
        return emptyState();
    return {
        version: LOCAL_STATE_VERSION,
...
const parseLocalCdnBaseManifest = (raw) => {
...
const readMemoryCdnBaseManifest = () => {
    try {
...
export const loadCdnBaseManifest = () => {
    const filePath = getCdnBaseManifestPath();
    if (!filePath)
        return readMemoryCdnBaseManifest();
    return parseLocalCdnBaseManifest(readJson(filePath));
};
export const saveCdnBaseManifest = (manifest) => {
    const filePath = getCdnBaseManifestPath();
    if (!filePath) {
        try {
...
export const listLocalPackageIds = () => {
...
export const loadLocalPackage = (styleId) => {
...
export const saveLocalPackage = (manifest, assets) => {
...
export const removeLocalPackage = (styleId) => {
    const dir = getStylePackageDir(styleId);
...
export const upsertUserPreset = (preset) => {
    const state = loadLocalState();
    const idx = state.userPresets.findIndex((p) => p.id === preset.id);
    if (idx >= 0)
        state.userPresets[idx] = preset;
    else
        state.userPresets.push(preset);
    saveLocalState(state);
};
export const removeUserPreset = (presetId) => {
    const state = loadLocalState();
...
const writeMemoryUserControls = (docs) => {
    localStorage.setItem(MEMORY_USER_CONTROLS_KEY, JSON.stringify(docs));
};
export const loadUserControlsDocument = (presetId) => {
...
export const loadUserControlsDefinition = (presetId) => {
...
export const saveUserControls = (presetId, doc) => {
    const filePath = getUserControlsPath(presetId);
    if (!filePath) {
...
export const removeUserControls = (presetId) => {
    const dir = getUserStyleDir(presetId);
    if (!dir || !isCepFs() || !fs.existsSync?.(dir)) {
        const mem = readMemoryUserControls();
        delete mem[presetId];
...
export const hasUserControls = (presetId) => {
    const filePath = getUserControlsPath(presetId);
    if (!filePath)
        return !!readMemoryUserControls()[presetId];
    try {
