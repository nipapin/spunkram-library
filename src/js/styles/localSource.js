 ...
 */
import { getBrand, storageKey } from "@brands";
...
const cepFsOk = () => typeof fs?.existsSync === "function" &&
...
export const getStoredCaptionsLocalRoot = () => {
    try {
...
export const setCaptionsLocalRoot = (folder) => {
...
const isAdminNow = () => isReleaseAdminEmail(getUserIdentity().email);
/**
 ...
 */
export const getCaptionsLocalRoot = (brand) => {
    if (!isAdminNow() || !cepFsOk())
        return null;
...
export const isCaptionsLocalOverrideActive = (brand) => {
    if (!getStoredCaptionsLocalRoot())
        return false;
    return !!getCaptionsLocalRoot(brand);
};
/** Same segment rules as `publicCaptionFileUrl` — styleId + fileName under local root. */
export const captionsLocalFile = (styleId, fileName, brand) => {
 ...
 */
export const resolveLocalPackPath = (packId, file, brand) => {
    const root = getCaptionsLocalRoot(brand);
    if (!root || !cepFsOk() || !packId)
        return null;
...
export const getLocalOverrideAssetPaths = (packIdOrStyleId, brand) => {
    const root = getCaptionsLocalRoot(brand);
    if (!root || !packIdOrStyleId)
        return null;
...
const packHasFile = (root, packId, file) => {
    const fileName = packProjectFileName(packId, file);
...
const applyFileFlag = (flags, file) => {
    if (file === "thumb.png")
        flags.thumb = true;
    else if (file === "preview.mp4")
        flags.preview = true;
    else if (file === "controls.json")
        flags.controls = true;
 ...
 */
export const scanLocalCaptionsCatalog = (brand) => {
    const root = getCaptionsLocalRoot(brand);
    if (!root || !cepFsOk()) {
    ...
    const ensure = (category, caption) => {
        ...
        if (!captions.has(caption))
            captions.set(caption, emptyFlags());
    ...
    for (const categoryName of categoryOrder) {
        ...
        const captionNames = Array.from(captionsMap.keys()).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
        ...
        for (const captionName of captionNames) {
            ...
            if (!flags.controls && !flags.thumb && !flags.preview)
                continue;
            ...
            const styleDir = categoryName === FLAT_CATEGORY && !fs.existsSync(dir) && fs.existsSync(flatDir)
                ? flatDir
                : dir;
            captions.push({
                id,
...
export const readLocalBaseManifest = (brand) => {
    const root = getCaptionsLocalRoot(brand);
    if (!root || !cepFsOk())
        return null;
...
export const readLocalControlsFile = (filePath) => {
    if (!cepFsOk() || !filePath || !fs.existsSync(filePath))
        return null;
    try {
...
export const isLocalFsPath = (url) => isAbsPath(url) && !url.startsWith("http");
/**
 * Read pack template or per-style definition.
 * For mogrt/aep, `id` is the pack name (POST body), e.g. `"Bounce"`.
 */
export const readLocalProjectFile = (id, file, brand) => {
    if (file === "definition") {
