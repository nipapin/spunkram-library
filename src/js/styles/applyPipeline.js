...
const fileExists = (p) => {
    if (!p || typeof fs?.existsSync !== "function")
        return false;
    try {
 ...
 */
export const getLocalStyleAssetPaths = (styleId) => {
...
const hostHasNeededFile = (paths, hostAppId) => {
    if (!paths)
        return false;
    if (hostAppId === "AEFT")
        return !!paths.aep || !!paths.mogrt;
    if (hostAppId === "PPRO")
        return !!paths.mogrt || !!paths.aep;
    return !!(paths.mogrt || paths.aep);
};
const presetFromLocal = (target, definition, version) => {
 ...
 */
export const acquirePresetProject = async (target, options) => {
    ...
    if (!options?.forceDownload && hasLocalPackTemplate(packId, hostAppId) && hostHasNeededFile(paths, hostAppId)) {
        ...
        await ensureCaptionFontsInstalled(target.styleId, definition).catch(() => 0);
        return {
    ...
    await ensureCaptionFontsInstalled(target.styleId, resolvedDefinition).catch(() => 0);
    return {
        preset,
 ...
 */
export const applyPresetProjectInHost = async (prepared) => {
...
export const acquireAndApplyPreset = async (target, options) => {
