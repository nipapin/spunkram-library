...
import { CAPTIONS_ENDPOINTS, apiUrl, getUserIdentity } from "../api";
...
import { captionsLocalFile, isCaptionsLocalOverrideActive, isLocalFsPath, readLocalBaseManifest, readLocalControlsFile, readLocalProjectFile, scanLocalCaptionsCatalog, } from "./localSource";
...
const asCaption = (raw, categoryName, categorySlug) => {
    if (!raw || typeof raw !== "object")
        return null;
    const o = raw;
    if (typeof o.id !== "string" || typeof o.name !== "string")
        return null;
    const files = (o.files && typeof o.files === "object" ? o.files : {});
    return {
        id: o.id,
        name: o.name,
...
const captionsCdnPrefix = (brand) => getBrand(brand).captionsCdnPrefix;
...
export const siblingPublicUrl = (url, fileName) => {
    if (!url)
        return null;
...
export const publicCaptionFileUrl = (styleId, fileName, brand) => {
    const segs = [
...
export const captionsCdnBaseManifestUrl = (brand) => publicCaptionFileUrl(CAPTIONS_CDN_VERSION_FOLDER, "manifest.json", brand);
/** GET CDN catalog version. Returns null on network / parse / CORS errors. */
export const fetchCaptionsCdnBaseManifest = async (brand) => {
    ...
    try {
        const response = await fetchWithTimeout(url, { method: "GET", cache: "no-store", headers: { ...NO_CACHE_HEADERS } }, CATALOG_TIMEOUT_MS);
        if (!response.ok)
            return null;
...
export const resolveControlsUrl = (item, brand) => {
    if (isCaptionsLocalOverrideActive(brand)) {
        if (item.controlsUrl && isLocalFsPath(item.controlsUrl))
    ...
    if (explicit && !isLocalFsPath(explicit))
        return explicit;
    if (item.controlsUrl && isLocalFsPath(item.controlsUrl))
        return item.controlsUrl;
    return (siblingPublicUrl(item.previewImageUrl, CONTROLS_FILE) ||
        siblingPublicUrl(item.previewVideoUrl, CONTROLS_FILE) ||
        publicCaptionFileUrl(item.id, CONTROLS_FILE, brand));
};
const controlsApiFallbackUrl = (cdnUrl, brand) => {
    try {
...
export const fetchCaptionControls = async (url, brand) => {
    const parse = (text) => {
        try {
...
export const fetchCaptionsCatalog = async (brand) => {
    ...
    const categories = Array.isArray(data.categories)
        ? data.categories.map(asCategory).filter((x) => !!x)
        : [];
    return {
...
export const flattenCatalog = (res) => res.categories.flatMap((c) => c.captions);
/** Fast non-crypto fingerprint for comparing local vs remote project bytes. */
export const hashArrayBuffer = (buffer) => {
...
const parseFilename = (disposition, id, file) => {
    if (disposition) {
...
const downloadPackFromPublicCdn = async (id, file, brand) => {
    if (file !== "aep" && file !== "mogrt")
        return null;
 ...
 */
export const downloadCaptionProject = async (id, file = "mogrt", user, brand, onlyIfChanged) => {
    if (isCaptionsLocalOverrideActive(brand)) {
        const local = readLocalProjectFile(id, file, brand);
        if (local) {
            if (onlyIfChanged?.contentHash &&
                local.contentHash &&
    ...
    const headerLength = typeof lenHeader === "string" && /^\d+$/.test(lenHeader) ? Number(lenHeader) : undefined;
...
export const pickProjectFile = (files, hostAppId) => {
    if (hostAppId === "AEFT") {
        if (files.aep)
            return "aep";
        if (files.mogrt)
...
export const fetchCaptionStylesCatalog = async () => {
