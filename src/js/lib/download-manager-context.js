...
import { downloadAndInstallOrUpdatePack, hasCachedPackZip, installCachedPack, installedPackMatchesMarketItem, } from "@/api/cep-market";
...
const DownloadManagerContext = globalThis[DOWNLOAD_MANAGER_CONTEXT_KEY] ??
    createContext(null);
globalThis[DOWNLOAD_MANAGER_CONTEXT_KEY] =
    DownloadManagerContext;
export function DownloadManagerProvider({ children, onPackReady, }) {
    ...
    const readyHandlers = useRef(new Map());
    /** Prefer cache install when retrying a failed job. */
    const preferCacheRef = useRef(new Set());
    const updateJob = useCallback((id, patch) => {
        setJobs((prev) => prev.map((j) => (j.id === id ? { ...j, ...patch } : j)));
    }, []);
    const releaseIfActive = useCallback((jobId) => {
    ...
    const pump = useCallback(async () => {
        ...
        const jobSnap = await new Promise((resolve) => {
            setJobs((prev) => {
        ...
        if (!jobSnap || cancelledRef.current.has(nextId)) {
            if (releaseIfActive(nextId))
                void pump();
            return;
        }
        const installedList = readInstallablePackages();
        const installedMeta = installedList.find((p) => installedPackMatchesMarketItem(p, jobSnap.pack));
        const preferCache = !installedMeta &&
        ...
        try {
            ...
            if (cancelledRef.current.has(nextId) ||
                ...
                showStatus(installedMeta ? `Updated: ${jobSnap.pack.name}` : `Installed: ${jobSnap.pack.name}`, "success", 4000);
    ...
    const enqueue = useCallback((pack, opts) => {
        const id = `dl-${pack.id}-${Date.now()}`;
        const useWhenReady = opts?.useWhenReady !== false;
        const cached = hasCachedPackZip(pack);
        if (opts?.onReady)
            readyHandlers.current.set(id, opts.onReady);
        if (cached)
            preferCacheRef.current.add(id);
        setJobs((prev) => [
            {
                id,
                pack,
        ...
        showStatus(cached ? `Installing from cache: ${pack.name}` : `Queued: ${pack.name}`, "info", 2500);
        void pump();
        return id;
    }, [pump, showStatus]);
    ...
    const dismissPackJobs = useCallback((packId) => {
        const id = String(packId);
        setJobs((prev) => prev.filter((j) => {
            if (String(j.pack.id) !== id)
                return true;
    ...
    const setUseWhenReady = useCallback((jobId, value) => {
        setJobs((prev) => prev.map((j) => (j.id === jobId ? { ...j, useWhenReady: value } : j)));
    }, []);
    ...
    const activeCount = useMemo(() => jobs.filter((j) => j.status === "queued" ||
        j.status === "downloading" ||
        j.status === "installing").length, [jobs]);
    const value = useMemo(() => ({
        jobs,
        activeCount,
        enqueue,
    ...
    return (_jsx(DownloadManagerContext.Provider, { value: value, children: children }));
}
