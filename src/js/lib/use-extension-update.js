...
import { applyExtensionUpdate, finalizePendingNativeUpdate, hasPendingNativeUpdate, } from "@/utils/extension-update";
import { pendingNativesOnly } from "@/utils/replace-live-file";
import { getEffectiveLocalVersion, readCrossHostAppliedVersion, reloadPanelHard, } from "@/utils/extension-version";
import { CROSS_HOST_UPDATE_POLL_MS, shouldReloadExtensionForAppliedUpdate, } from "@/utils/cross-host-update";
...
export function useExtensionUpdate() {
    ...
    const [updateZxpUrl, setUpdateZxpUrl] = useState(null);
    ...
    const applyRemoteInfo = useCallback((info, opts) => {
    ...
    useEffect(() => {
        try {
            ...
            if (nativeLeft.length > 0) {
                showStatus("Restart Premiere Pro / After Effects to finish the native plugin update.", "info", 12000);
            }
        }
        catch (err) {
            console.warn("[extension-update] pending native finalize failed:", err instanceof Error ? err.message : err);
    ...
    useEffect(() => {
        if (typeof window === "undefined")
            return;
        const tick = () => {
            if (reloadingPanelRef.current)
                return;
            const applied = readCrossHostAppliedVersion();
            if (!shouldReloadExtensionForAppliedUpdate({
    ...
    useEffect(() => {
        ...
        fetchUpdateInfo().then((info) => {
            if (cancelled || !info?.version || !info.zxpUrl)
                return;
            applyRemoteInfo({
    ...
    const handleApplyUpdate = useCallback(async () => {
        if (!updateZxpUrl || !updateVersion || updateBusy)
            return;
        setUpdateBusy(true);
        setUpdateError(null);
        ...
        try {
            ...
            if (result.pendingNatives.length > 0) {
                ...
                showStatus("Panel updated. Restart Premiere Pro / After Effects to finish the native plugin update.", "info", 12000);
            }
        }
        catch (err) {
            setUpdateBusy(false);
