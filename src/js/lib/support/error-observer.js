...
function extraCode(extra) {
    if (!extra)
        return undefined;
    for (const key of ["error_code", "api_code", "reason", "code"]) {
        const v = extra[key];
...
function httpStatusFrom(message, extra) {
    const raw = extra?.http_status;
    if (typeof raw === "number" && Number.isFinite(raw))
        return raw;
    if (typeof raw === "string" && /^\d+$/.test(raw))
        return Number(raw);
    const m = message.match(/\bHTTP[_ ](\d{3})\b/i) ||
        message.match(/\b(?:download|diff download) failed\s*\((\d{3})\)/i) ||
        message.match(/\((\d{3})\)\s*$/);
    return m ? Number(m[1]) : null;
}
function errnoFrom(message) {
    const m = message.match(/\b(E(?:TIMEDOUT|SOCKETTIMEDOUT|CONNREFUSED|CONNRESET|CONNABORTED|NOTFOUND|PIPE|PROTO|HOSTUNREACH|NETUNREACH|NETDOWN|AI_AGAIN|ACCES|PERM|NOENT|NOSPC))\b/);
    return m?.[1];
}
/**
 ...
 */
function isNonCritical(message, code, extra) {
    const trimmed = message.trim();
    if (NON_CRITICAL_CODES.has(trimmed.toUpperCase()))
        return true;
    const codes = [code, extraCode(extra), errnoFrom(message)].filter((c) => Boolean(c));
    if (codes.some((c) => NON_CRITICAL_CODES.has(c.toUpperCase())))
        return true;
    const status = httpStatusFrom(message, extra);
    if (status != null && SKIP_HTTP_STATUSES.has(status))
        return true;
    const blob = [message, ...codes].join("\n");
    return NON_CRITICAL_PATTERNS.some((re) => re.test(blob));
}
function isGlobalNoise(message, extra) {
    if (GLOBAL_NOISE_PATTERNS.some((re) => re.test(message)))
        return true;
    if (!HMR_CONTEXT_PATTERN.test(message))
        return false;
...
function dedupeKey(severity, action, message) {
    return `${severity}::${action}::${message}`;
}
/** True if we already successfully sent this report recently. */
function wasRecentlySent(severity, action, message) {
...
function markSent(severity, action, message) {
    const key = dedupeKey(severity, action, message);
...
function logResult(result) {
    if (!result.ok) {
        console.warn("[support] report failed →", SUPPORT_REPORT_URL, result.status, result.error, (result.text || "").slice(0, 200));
    }
}
/**
 ...
 */
export function reportError(action, error, opts) {
    try {
        ...
        if (severity !== "error")
            return Promise.resolve();
        if (isGlobalNoise(parts.message, {
            ...extra,
            stack: extra?.stack ?? parts.stack ?? null,
        })) {
            return Promise.resolve();
        }
        if (isNonCritical(parts.message, parts.code, extra))
            return Promise.resolve();
...
export function reportWarning(action, error, opts) {
    void reportError(action, error, { ...opts, severity: "warning" });
}
/** Explicit info helper (never Telegram). */
export function reportInfo(action, error, opts) {
...
export function installGlobalHandlers() {
    ...
    window.addEventListener("error", (event) => {
        ...
        if (isGlobalNoise(parts.message, extra) ||
    ...
    window.addEventListener("unhandledrejection", (event) => {
        ...
        if (isGlobalNoise(parts.message, { stack: parts.stack ?? null }) ||
            isNonCritical(parts.message, parts.code)) {
            return;
        }
