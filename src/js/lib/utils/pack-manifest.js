...
export function resolvePackBundleDir(packFilePath) {
    return path.dirname(packFilePath);
}
export function packManifestPath(packDir) {
    return path.join(packDir, "manifest.json");
}
export function readLocalPackManifest(packDir) {
...
export function writeLocalPackManifest(packDir, manifest) {
    fs.writeFileSync(packManifestPath(packDir), JSON.stringify(manifest, null, 2), "utf8");
}
export function normalizePackManifest(raw) {
    const map = new Map();
    if (raw == null)
        return map;
...
export function removeFilesNotInManifest(packDir, oldManifest, newManifest) {
