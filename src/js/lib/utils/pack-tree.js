...
export function instanceGroupJoin(pathSegments) {
    return pathSegments.join(INSTANCE_GROUP_JOIN_CHAR);
}
export function instanceGroupSplit(viewId) {
    if (!viewId)
        return [];
    return viewId.split(INSTANCE_GROUP_JOIN_CHAR).filter(Boolean);
}
function isLeafGroup(node) {
    return (!!node &&
        typeof node === "object" &&
        "preview" in node &&
...
export function normalizeItemPlans(raw) {
...
function nodeIsAudio(node) {
    return !!node.is_audio;
}
function processNode(node, label, pathSegments, inheritedAudio = false) {
    const isAudio = inheritedAudio || nodeIsAudio(node);
    if (isLeafGroup(node)) {
        const group = isAudio && !node.is_audio ? { ...node, is_audio: true } : node;
        const items = buildItems(group, pathSegments);
        const enabledItems = items.filter((item) => item.enabled);
        const count = enabledItems.length;
        const isNew = !!group.is_new_mark;
        const premiumCount = group.premium || group.enabled_only
 ...
 */
export function buildPackTree(structure) {
...
export function flattenPackGroups(nodes) {
    const groups = [];
    const walk = (list) => {
 ...
 */
export function collectContentSections(node) {
    if (node.kind === "group") {
        return [
            {
                id: node.id,
 ...
 */
export function collectAllContentSections(tree) {
 ...
 */
export function filterContentSections(sections, query) {
...
export function filterFavoriteSections(sections, favoriteIds) {
    return sections
        .map((section) => ({
        ...section,
...
export function filterUnlockedSections(sections, isUnlocked) {
    return sections
        .map((section) => ({
        ...section,
 ...
 */
export function filterPackTree(nodes, keepItem) {
    const out = [];
...
export function getFirstPackRoot(nodes) {
    return nodes[0];
}
/** Find a node by viewId / id. */
export function findPackTreeNode(nodes, id) {
    for (const node of nodes) {
 ...
 */
export function findPackRootFor(nodes, id) {
    if (!id)
        return undefined;
...
export function firstGroupIdIn(node) {
    if (node.kind === "group")
        return node.id;
...
export function getFirstPackGroup(nodes) {
 ...
 */
export function resolveItemAssetSegments(item) {
    const { group, pathSegments, previewKey } = item;
    const customFolder = typeof group.custom_folder === "string" ? group.custom_folder : undefined;
    const leafFolder = customFolder || pathSegments[pathSegments.length - 1] || "";
