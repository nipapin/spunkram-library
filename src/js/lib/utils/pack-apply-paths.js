...
const AUDIO_EXTS = [".wav", ".mp3", ".ogg", ".m4a", ".aac", ".aif", ".aiff", ".flac"];
...
export function packItemIsAudio(item) {
    if (item.group.is_audio)
        return true;
    const filetype = customArgsFor(item).filetype;
...
function firstExistingAudioInDir(dir, stem, preferredExt) {
    if (!cepFsAvailable() || !dir || !stem)
        return null;
 ...
 */
export function resolveItemAudioFile(item, packFilePath) {
    if (!packItemIsAudio(item) || !packFilePath || typeof path?.join !== "function")
        return null;
    const preferredExt = preferredAudioExt(item);
    const stems = [audioStemFor(item), item.previewKey, item.name].filter((stem, i, arr) => !!stem && arr.indexOf(stem) === i);
    const segments = resolveItemAssetSegments(item);
    const previewGroupRel = segments.length > 0 ? path.dirname(path.join(...segments)) : "";
    const instanceGroupRel = item.pathSegments.length > 0 ? path.join(...item.pathSegments) : "";
    const rels = [instanceGroupRel, previewGroupRel].filter((rel, i, arr) => !!rel && rel !== "." && arr.indexOf(rel) === i);
    if (rels.length === 0)
        rels.push("");
    const roots = [
...
function withCustomFilesFolder(file, templatesDir, customFilesFolder) {
    if (typeof customFilesFolder !== "string" || !customFilesFolder)
        return file;
 ...
 */
export function resolveItemSourceFile(item, packFilePath, hostAppId, settings) {
    ...
    if (group.is_presets) {
        if (hostAppId === "AEFT") {
            ...
            const rawExt = typeof customArgs.filetype === "string" && customArgs.filetype.trim()
    ...
    if (group.is_footage) {
        const ext = typeof customArgs.filetype === "string"
            ? customArgs.filetype
    ...
    const groupSourceType = typeof group.custom_source_type === "string" && group.custom_source_type
        ? group.custom_source_type
        : undefined;
    const rawSourceType = customArgs.custom_source_type ||
        groupSourceType ||
        settings?.inside_option_sets?.source_type ||
        "FULL_PROJECT";
    const sourceType = rawSourceType === "PROJECT" ? "FULL_PROJECT" : rawSourceType;
    if (sourceType === "MOGRT") {
        const stem = (typeof customArgs.multiuse_ref_mogrt === "string" &&
            customArgs.multiuse_ref_mogrt) ||
            (group.preview_name_instead_id ? item.name : item.previewKey);
        const file = withCustomFilesFolder(path.join(groupDir, `${stem}.mogrt`), templatesDir, customArgs.custom_files_folder);
        return {
            ctype: "MOGRT",
            file,
    ...
    const file = withCustomFilesFolder(path.join(groupDir, `${lastGroup}.prproj`), templatesDir, customArgs.custom_files_folder);
    return {
        ctype: "FULL_PROJECT",
        file,
