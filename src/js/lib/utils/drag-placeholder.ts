...
const PLACEHOLDER_PNG_BASE64 =
  "iVBORw0KGgoAAAANSUhEUgAAAEAAAAAkCAIAAAC2bqvFAAAAU0lEQVR4nO3PwQkAIBDAsBvUURzdh0P4CEKhA6Sz1/m64YIGtKABLWhACxrQgga0oAEtaEALGtCCBrSgAS1oQAsa0IIGtKABLWhACxrQgga04LEL2LZ08T95f6oAAAAASUVORK5CYII=";

export function ensureDragPlaceholderFile(): string | null {
  try {
...
function adoptMessage(reason: unknown): string {
...
export async function releaseDragAnchor(): Promise<void> {
  try {
...
export async function adoptTimelinePlaceholder(): Promise<
  { ok: true } | { ok: false; message: string }
> {
  const first = await evalTS("adoptDragPlaceholder", DRAG_PLACEHOLDER_NAME, false);
  if (first && first.ok) return { ok: true };
  ...
  if (second && second.ok) return { ok: true };
