...
function findEndOfCentralDirectoryInBuffer(buf) {
...
function findEndOfCentralDirectory(fd, fileSize) {
    ...
    if (cdOffset === 0xffffffff || cdSize === 0xffffffff || totalEntries === 0xffff) {
        throw new Error("ZIP64 archives are not supported yet. Re-export the pack or contact support.");
    }
    return { eocdOffset, totalEntries, cdSize, cdOffset };
}
function parseCentralDirectory(cdBuf, totalEntries) {
    const entries = [];
    let cdOffset = 0;
    for (let i = 0; i < totalEntries; i++) {
        ...
        if (compressedSize === 0xffffffff ||
            ...
            throw new Error(`ZIP64 entry is not supported yet: "${name}". Re-export the pack or contact support.`);
        }
        entries.push({
            name,
            isDir: name.endsWith("/"),
...
function readZipEntryDataFromFd(fd, entry) {
...
export function extractZipToFolder(zipPath, destDir, opts) {
