...
import { fetchGalAssetsManifest, fetchGalEffectsFileLink, } from "@/api/gal-effects";
import { downloadToFile } from "@/utils/download-file";
import { getSessionToken } from "@/lib/api/session";
import { normalizePackManifest, } from "@/lib/utils/pack-manifest";
import { resolveItemSourceFile, } from "@/lib/utils/pack-apply-paths";
...
function hostFromPackHost() {
    return currentPackHost() === "AE" ? "AE" : "PR";
}
function hostAppIdFromGal(host) {
 ...
 */
export function resolveGalInstallRoot(host = hostFromPackHost()) {
    if (!cepFsAvailable())
        return null;
    try {
...
export function resolveGalPackStubPath(installRoot) {
    return path.join(installRoot, STUB_PACK_NAME);
}
export function resolveGalAssetsPath(installRoot) {
    return path.join(installRoot, "Assets");
}
export function resolveGalFileIndexPath(installRoot) {
    return path.join(installRoot, FILE_INDEX_NAME);
}
/** Ensure stub pack file + Assets dir so resolvePackTemplatesPath works. */
export function ensureGalVirtualPackLayout(installRoot) {
    ...
    try {
        ...
        if (!fs.existsSync(packFilePath)) {
            fs.writeFileSync(packFilePath, JSON.stringify({
                settings: {
                    main: {
...
export function readGalFileIndex(installRoot) {
    const empty = {
    ...
    try {
        ...
        return {
            ...
            files: raw.files && typeof raw.files === "object" && !Array.isArray(raw.files)
                ? raw.files
                : {},
        };
    }
...
export function writeGalFileIndex(installRoot, index) {
    if (!cepFsAvailable())
        return;
    try {
        if (!fs.existsSync(installRoot))
            fs.mkdirSync(installRoot, { recursive: true });
        fs.writeFileSync(resolveGalFileIndexPath(installRoot), JSON.stringify(index, null, 2), "utf8");
    }
    catch {
        /* ignore */
    }
...
function stripToTemplatesRel(rel) {
    let p = normalizeRel(rel);
    if (p === R2_PROJECTS_PREFIX)
        return "";
 ...
 */
export function canonicalizeGalAssetsRelPath(rel) {
    ...
    if (lower === "_assets")
        return null;
    if (lower === `${R2_PROJECTS_PREFIX.toLowerCase()}/_assets`)
        return null;
...
export function isGalAssetsMediaPath(rel) {
 ...
 */
export function galRelPathToDisk(installRoot, rel) {
    const rest = stripToTemplatesRel(rel);
    if (!rest)
        return resolveGalAssetsPath(installRoot);
    return path.join(installRoot, "Assets", ...rest.split("/"));
}
/**
 * Absolute disk file under local Assets → R2 path under `Projects/`.
 */
export function galDiskToRelPath(installRoot, absoluteFile) {
...
function galProjectPathCandidates(rel) {
...
function indexEntryFor(index, rel) {
    const n = normalizeRel(rel);
    const rest = stripToTemplatesRel(n);
    const keys = [
        n,
...
function filterAssetsManifest(map) {
    const out = new Map();
    for (const [rel, hash] of map) {
        ...
        if (!out.has(canonical))
            out.set(canonical, hash);
    }
    return out;
}
function localFileLooksCurrent(diskPath, entry, expectedHash) {
    if (!cepFsAvailable() || !fs.existsSync(diskPath))
        return false;
    ...
    if (!expectedHash && entry?.etag) {
        try {
            ...
            if (entry.size != null && st.size !== entry.size)
                return false;
            return true;
        }
        catch {
...
export async function syncGalAssetsFromManifest(opts) {
    ...
    const remoteMap = filterAssetsManifest(normalizePackManifest(remote.data.manifest));
    const toDownload = [];
    for (const [rel, hash] of remoteMap) {
        const disk = galRelPathToDisk(installRoot, rel);
        const entry = indexEntryFor(index, rel);
        if (localFileLooksCurrent(disk, entry, hash))
            continue;
    ...
    if (toDownload.length === 0) {
        ...
        if (opts.packVersion)
            index.pack_version = opts.packVersion;
        if (opts.packEtag)
            index.pack_etag = opts.packEtag;
...
export async function ensureGalItemOnDisk(opts) {
    ...
    const installRoot = resolveGalInstallRoot(host);
    if (!installRoot)
        return { ok: false, error: "NO_INSTALL_ROOT" };
    const { packFilePath, assetsPath } = ensureGalVirtualPackLayout(installRoot);
    const appId = hostAppIdFromGal(host);
    let resolved;
    try {
        resolved = resolveItemSourceFile(opts.item, packFilePath, appId, opts.settings);
    }
    catch (e) {
        return {
            ok: false,
    ...
    const sizeOk = linkData.size == null ||
        !exists ||
        (() => {
            try {
...
export function galOfflineAssetsPresent(installRoot) {
    if (!installRoot || !cepFsAvailable())
        return false;
...
export function galItemLocallyCached(item, settings, host = hostFromPackHost()) {
    const installRoot = resolveGalInstallRoot(host);
    if (!installRoot)
        return false;
    const packFilePath = resolveGalPackStubPath(installRoot);
    if (!fs.existsSync(packFilePath))
        return false;
    try {
        const resolved = resolveItemSourceFile(item, packFilePath, hostAppIdFromGal(host), settings);
        if (!resolved || resolved.ctype === "UNSUPPORTED")
            return false;
