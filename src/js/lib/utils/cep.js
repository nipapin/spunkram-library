...
export const textCepPatch = (e) => {
    const isMac = os.platform() === "darwin";
    if (!isMac)
        return; // Only needed on MacOS, Windows handles this natively
    // console.log("keyup", e);
    ...
    if (e.key === "ArrowLeft") {
        ...
        if (end === input.value.length)
            return; // Prevents going to start
        if (isShiftKey) {
