...
type SessionKind = "placeholder" | "apply";
...
function dragGhost(): HTMLImageElement {
  if (!ghost) {
    ghost = new Image(1, 1);
    ghost.src = GHOST_SRC;
    ghost.alt = "";
    ...
    if (typeof document !== "undefined") document.documentElement.appendChild(ghost);
  }
  return ghost;
}

function hideDragGhost(transfer: DataTransfer): void {
  try {
...
function hostDragPath(filePath: string): string {
  try {
    if (os.platform() === "win32") return filePath.replace(/\//g, "\\");
  } catch {
...
function bindApplyListeners(): void {
  ...
  const onLeave = (event: DragEvent) => {
    if (event.relatedTarget == null) leftPanel = true;
  };
...
function armSession(event: DragStartEvent, next: SessionKind, filePath?: string): boolean {
  ...
  if (filePath) transfer.setData("com.adobe.cep.dnd.file.0", hostDragPath(filePath));
  else transfer.setData("application/x-spunkram-pack-item", "apply");
  return true;
}

/** Premiere places this file itself. No second apply on drag end. */
export function beginHostFileDrag(event: DragStartEvent, filePath: string): boolean {
  const transfer = event.dataTransfer;
  resetSession();
  if (!transfer || !filePath) return false;
  transfer.effectAllowed = "copy";
  hideDragGhost(transfer);
...
export function beginPlaceholderDrag(event: DragStartEvent, filePath: string): boolean {
  if (!filePath) return false;
  return armSession(event, "placeholder", filePath);
}

/** After Effects: release outside the panel applies at the current time. */
export function beginApplyOnDropOutside(event: DragStartEvent): void {
 ...
 */
export function finishHostDrag(event: DragEndEvent): "placeholder" | "apply" | null {
  ...
  resetSession();
  if (!should || !sessionKind || escaped || droppedHere || !releasedOutside) return null;
  return sessionKind;
}

if (typeof document !== "undefined") dragGhost();
