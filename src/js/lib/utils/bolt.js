...
let cachedDomHostAppId = null;
const normalizeCepHostId = (raw) => {
    const u = String(raw || "").toUpperCase();
    if (u === "AEFT" || u.indexOf("AEFT") === 0)
        return "AEFT";
    if (u === "PPRO" || u.indexOf("PPRO") === 0)
        return "PPRO";
    return null;
};
/** Clear DOM host cache — call before reloading ExtendScript. */
export function clearHostAppIdCache() {
 ...
 */
export async function probeHostAppIdFromDom() {
    if (!window.cep)
        return null;
    try {
...
export const evalES = (script, isGlobal = false) => {
    return new Promise(function (resolve, reject) {
        ...
        csi.evalScript("try{" + fullString + "}catch(e){try{$.writeln('[cep] '+e);}catch(_w){} JSON.stringify({evalESError:true,message:String(e&&e.message!=null?e.message:e)});}", (res) => {
            resolve(res);
        });
    });
};
 ...
 *
 */
export const evalTS = (functionName, ...args) => {
    return new Promise(function (resolve, reject) {
...
export const evalFile = (file) => {
    return evalES("typeof $ !== 'undefined' ? $.evalFile(\"" +
        file +
...
const evalHostJsxFile = async (jsxSrc) => {
 ...
 *
 */
export const listenTS = (event, callback, isLocal = true) => {
 ...
 * @param isLocal Whether to scope the event to the panel's namespace. Defaults to true
 */
export const dispatchTS = (event, data, scope = "APPLICATION", appId = csi.getApplicationID(), id = csi.getExtensionID(), isLocal = true) => {
    const fullEvent = isLocal ? `${ns}.${event}` : event;
...
export const initBolt = (log = true) => {
    ...
    boltInit = (async () => {
        ...
        await stampAndBindHost();
        const kind = await evalES(`(function(){
        var g = typeof $ !== "undefined" ? $ : this;
        var out = {
          kind: "",
          cep: "",
...
export const posix = (str) => str.replace(/\\/g, "/");
...
export const getAppBackgroundColor = () => {
    const { green, blue, red } = JSON.parse(window.__adobe_cep__.getHostEnvironment()).appSkinInfo.panelBackgroundColor.color;
    return {
        rgb: {
            r: red,
            g: green,
...
export const subscribeBackgroundColor = (callback) => {
    const getColor = () => {
...
export const vulcanSend = (id, msgObj) => {
    const msg = new VulcanMessage(VulcanMessage.TYPE_PREFIX + id, null, null);
    const msgStr = JSON.stringify(msgObj);
    msg.setPayload(msgStr);
    vulcan.dispatchMessage(msg);
};
export const vulcanListen = (id, callback) => {
    vulcan.addMessageListener(VulcanMessage.TYPE_PREFIX + id, (res) => {
...
export const selectFolder = (dir, msg, callback) => {
    const result = (window.cep.fs.showOpenDialogEx || window.cep.fs.showOpenDialog)(false, true, msg, dir);
...
export const selectFolderAsync = (dir, msg) => new Promise((resolve) => {
    try {
        const result = (window.cep.fs.showOpenDialogEx || window.cep.fs.showOpenDialog)(false, true, msg, dir);
        if (result.data?.length > 0) {
...
export const selectFile = (dir, msg, callback) => {
    const result = (window.cep.fs.showOpenDialogEx || window.cep.fs.showOpenDialog)(false, false, msg, dir);
    if (result.data?.length > 0) {
        const folder = decodeURIComponent(result.data[0].replace("file://", ""));
        callback(folder);
    }
