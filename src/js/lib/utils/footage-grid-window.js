import { resolvePreviewAspectRatio } from "./pack-tree";
...
export function cardWidthPx(metrics) {
...
export function layoutGridSections(sections, metrics) {
    const cols = Math.max(1, metrics.columns);
    ...
    for (const section of sections) {
        ...
        const aspect = parseAspectRatio(resolvePreviewAspectRatio(section.items[0].group));
        const rowHeight = cardWidth / Math.max(0.01, aspect);
        const stride = rowHeight + metrics.gap;
        const rows = Math.ceil(itemCount / cols);
        const gridHeight = rows === 0 ? 0 : rows * rowHeight + Math.max(0, rows - 1) * metrics.gap;
...
export function visibleRowsForSection(layout, viewTop, viewBottom) {
    ...
    const endRow = Math.min(layout.rows, Math.max(startRow + 1, Math.ceil((viewBottom - gridTop) / stride)));
    return { startRow, endRow };
}
export function windowGridSections(layout, scrollTop, viewportHeight, bufferPx = WINDOW_BUFFER_PX) {
    ...
    return layout.sections.map((section, sectionIndex) => {
        const { startRow, endRow } = visibleRowsForSection(section, viewTop, viewBottom);
        return {
            sectionIndex,
            startRow,
            endRow,
...
export function windowRangeKey(windows) {
    let key = "";
    for (const win of windows) {
        if (win.endRow <= win.startRow)
            continue;
