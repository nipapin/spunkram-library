...
function resolveUserFontsDir() {
    const isWin = os.platform?.() === "win32";
    if (isWin) {
        try {
            const userData = typeof csi?.getSystemPath === "function"
                ? csi.getSystemPath("userData")
                : "";
            if (userData) {
                // Beta: dirname(USER_DATA)/Local/Microsoft/Windows/Fonts
                return path.join(path.dirname(userData), "Local", "Microsoft", "Windows", "Fonts");
            }
        }
        catch {
            // fall through
...
export function userFontFileInstalled(fileName) {
    if (!cepFsAvailable() || !fileName)
        return false;
    try {
 ...
 */
export async function installFontsFromDirectory(fontsFolder) {
    if (!cepFsAvailable() || !fontsFolder || !fs.existsSync(fontsFolder))
        return 0;
    ...
    const isWin = os.platform?.() === "win32";
    const regPath = "HKCU\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts";
    let installed = 0;
    for (const font of fonts) {
        ...
        try {
            ...
            if (isWin && typeof child_process?.execSync === "function") {
                try {
                    child_process.execSync(`reg add "${regPath}" /f /t REG_SZ /v "${fontName}" /d "${targetFile}"`, { windowsHide: true, stdio: "ignore" });
                }
                catch {
 ...
 */
export async function installPackFonts(packFilePath) {
