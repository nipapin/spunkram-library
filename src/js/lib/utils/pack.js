...
import { currentPackHost, normalizePackHost, resolveInstalledPackHost, packMetaMatchesHost, } from "./pack-host";
 ...
 */
export function parsePackageFileFormat(filename, customFileType) {
    const match = /(?:\.([^.]+))?$/.exec(filename.toString());
    const format = match?.[1]?.toLowerCase();
    if (!format)
        return false;
    if (customFileType)
        return format === customFileType.toLowerCase();
    return PACKAGE_FILETYPES.includes(format);
}
/**
...
function looksLikeJsonPack(raw) {
    const cleaned = raw.replace(/^\uFEFF/, "").trim();
    return cleaned.startsWith("{");
}
function toInitResult(content) {
    return {
        method: "JSON",
 ...
 */
export function initPackage(packPath, initCallback, options) {
    if (!cepFsAvailable()) {
...
export function initPackageAsync(packPath, options) {
    return new Promise((resolve, reject) => {
        initPackage(packPath, (result, error) => {
            if (result)
                resolve(result);
            else
...
export function initPackageSync(packPath, options) {
...
export function packInitErrorMessage(error) {
...
export function resolvePackAssetsPath(packFilePath) {
    return resolvePackPreviewsPath(packFilePath);
}
/** Default `preferences.json` location for Motionflow CEP. */
export function getPreferencesCandidates() {
    const prefPath = resolvePreferencesPath();
    return prefPath ? [prefPath] : [];
}
export function readInstalledPackagesFromPreferences(preferencesPath) {
    if (!cepFsAvailable() || typeof fs.readFileSync !== "function")
        return [];
...
export function readAllInstallablePackages() {
    return readInstalledPackagesFromPreferences().filter((p) => p.path && parsePackageFileFormat(p.path));
}
/**
 * Installed packs for a host. Defaults to the current CEP host.
 * Packs with unknown/mismatched `appID` are excluded.
 */
export function readInstallablePackages(host = currentPackHost()) {
...
export async function loadInstalledPack(meta) {
    const host = currentPackHost();
    const pack = await initPackageAsync(meta.path);
    const resolvedHost = resolveInstalledPackHost(meta) ||
 ...
 */
export async function loadFirstInstalledPack() {
