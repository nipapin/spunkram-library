 ...
 */
export function getResolvedHostSync() {
    return resolvedHost;
}
/**
 ...
 */
export function isAfterEffects() {
...
async function probeHostDom() {
    ...
    try {
        ...
        if (parsed.hasRQ === true)
            return "AEFT";
        // sequences / rootItem.children is PPro-only
        if (parsed.hasSeq === true)
            return "PPRO";
 ...
 */
export async function getResolvedHostAppId() {
    if (resolvedHost)
        return resolvedHost;
    if (!probePromise) {
        probePromise = probeHostDom().then((host) => {
            ...
            if (cepId && cepId !== host) {
                console.warn(`[host-identity] CSInterface reports "${cepId}" but DOM probe detected "${host}". Using "${host}".`);
            }
            return host;
        });
    }
 ...
 */
export async function isAfterEffectsAsync() {
 ...
 */
export async function isPremiereProAsync() {
    const host = await getResolvedHostAppId();
    return host === "PPRO";
}
/**
 ...
 */
export function initHostIdentity() {
    if (!window.cep)
        return;
