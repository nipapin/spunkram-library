 ...
 */
export function resolvePackTemplatesPath(packFilePath, _hostAppId) {
...
export function resolvePackPreviewsPath(packFilePath) {
    if (typeof path?.dirname !== "function" || typeof path?.join !== "function") {
        return "";
    }
...
export function resolvePackFontsPath(packFilePath) {
    if (typeof path?.dirname !== "function" || typeof path?.join !== "function") {
        return null;
    }
    const fontsDir = path.join(path.dirname(packFilePath), "Fonts");
    if (cepFsAvailable() && fs.existsSync(fontsDir))
        return fontsDir;
    return null;
}
/**
 ...
 */
export function resolveFullProjectAssetsPath(templatesDir, item) {
    const folder = (typeof item.group.custom_assets_folder === "string" &&
    ...
    if (folder) {
        ...
        if (cepFsAvailable() && fs.existsSync(specific))
            return specific;
    }
    if (cepFsAvailable() && fs.existsSync(assetsRoot))
        return assetsRoot;
