...
import { resolveItemSourceFile, resolvePackTemplatesPath, } from "./pack-apply-paths";
import { applyFullProjectViaCopyPaste, resolveFullProjectAssetsPath, } from "./copy-paste-apply";
...
export async function applyPackItemToHost(item, packFilePath, settings) {
        ...
        try {
            ...
            if (data && typeof data === "object" && "ok" in data && data.ok === false) {
                ...
                if (reason === "NO_INOUT") {
                    ...
                    return {
                        ok: false,
                        message: friendlyReason(reason || (appId === "PPRO" ? "NO_ACTIVE_SEQUENCE" : "COMP")),
                    };
                }
            }
        }
    ...
    const previewEntry = item.group.preview?.[item.previewKey];
    const customArgs = previewEntry?.custom_args || {};
    const composerPayload = appId === "AEFT"
        ? {
            itemId: item.id,
            instanceGroup: item.pathSegments.join(INSTANCE_GROUP_JOIN_CHAR),
            argsObject: {
                ...(previewEntry || { name: item.name }),
                label_color_num: previewEntry?.label_color_num ?? 2,
                parent_folder: previewEntry?.parent_folder ?? false,
            ...
            packOptions: settings?.inside_option_sets || {},
        }
        : undefined;
    if (appId === "AEFT" && composerPayload) {
        ...
        if (route === "text_animator") {
            try {
                const wrapped = await Motionflow.AE.addTextAnimator("APPLY", composerPayload.argsObject, composerPayload.extraArguments, composerPayload.templatesDir, composerPayload.packName, composerPayload.packOptions);
                if (!wrapped.ok) {
        ...
        if (route === "photo_animator") {
            try {
                const wrapped = await Motionflow.AE.addPhotoAnimator("APPLY", composerPayload.argsObject, composerPayload.extraArguments, composerPayload.templatesDir, composerPayload.packName, composerPayload.packOptions);
                if (!wrapped.ok) {
    ...
    try {
        const wrapped = await Motionflow.applyPackItem({
            ...
            blendMode: typeof customArgs.layer_blendmode === "string" && customArgs.layer_blendmode.trim()
