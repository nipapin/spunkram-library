...
export const SPUNKRAM_DEV_PLANS = ["free", "purchased", "subscribed"];
export function isSpunkramDevPlan(value) {
    return value === "free" || value === "purchased" || value === "subscribed";
}
export function spunkramDevPlanLabel(plan) {
    if (plan === "purchased")
        return "Purchased";
    if (plan === "subscribed")
        return "Subscribed";
...
export function applySpunkramAdminDevPlan(status, plan) {
    if (!isSpunkramDevPlan(plan))
        return status;
    if (plan === "subscribed") {
        ...
        return {
            ...
            status: "active",
            tier: "subscribed",
            aiGenerationsLimit: typeof serverLimit === "number" && serverLimit > 0 ? serverLimit : 100,
        };
    }
    if (plan === "purchased") {
        return {
