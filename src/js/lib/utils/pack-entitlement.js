...
export function installedPackMatchesMarketItem(meta, item) {
    ...
    if (metaApp && itemApp && metaApp !== itemApp)
        return false;
    const installedName = (meta.name || "").trim();
    if (!installedName)
        return false;
    const catalogNames = [item.pack_name, item.name].filter(Boolean);
    if (catalogNames.some((n) => packNamesMatch(installedName, n)))
        return true;
...
function purchaseMatchesPack(purchase, meta) {
    ...
    const purchaseHost = purchase.primary_type
        ...
        : null;
    if (metaHost && purchaseHost && metaHost !== purchaseHost)
        return false;
    const purchaseName = purchase.name || "";
    const metaName = meta.name || "";
    if (!purchaseName || !metaName)
        return false;
...
function catalogItemIsFree(item) {
    const action = (item.action || "").toLowerCase();
    if (action === "get_free")
        return true;
    const price = typeof item.custom_price === "number"
        ? item.custom_price
 ...
 */
export function isPackEntitled(meta, ctx) {
    if (!ctx?.signedIn)
        return false;
    ...
    if (item) {
        if (item.owned || item.covered_by_subscription)
            return true;
        if (catalogItemIsFree(item))
            return true;
    }
    if (ctx.purchases.some((p) => purchaseMatchesPack(p, meta)))
        return true;
    return false;
}
