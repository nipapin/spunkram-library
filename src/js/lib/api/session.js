 ...
 */
import { readActiveMotionflowAuth, removeAccountSession, writeMotionflowAuth, } from "@/lib/api/preferences";
...
let lastSentSessionToken = null;
let pendingWipeTimer = null;
...
export function sessionAuthHeaders(extra) {
    const headers = {
...
export function onSessionReload(listener) {
 ...
 */
export function clearSession(reason = "UNAUTHORIZED", failedToken) {
    const failed = failedToken ?? lastSentSessionToken;
    const disk = readActiveMotionflowAuth();
    if (!shouldWipeSharedVault({
        failedToken: failed,
...
function confirmUnauthorizedWipe(reason, failedToken) {
 ...
 */
export function handleUnauthorized(reason = "UNAUTHORIZED", failedToken) {
    ...
    if (pendingWipeTimer && pendingWipeFailed === failedKey)
        return;
    if (pendingWipeTimer)
        clearTimeout(pendingWipeTimer);
