...
function xhrRequest(url, init = {}) {
    return new Promise((resolve) => {
        try {
            ...
            xhr.ontimeout = () => resolve({ ok: false, text: "", status: 0, error: "TIMEOUT" });
            xhr.onerror = () => resolve({ ok: false, text: "", status: 0, error: "NO_CONNECTION" });
            xhr.onload = () => {
 ...
 */
export async function cepHttpRequest(url, init = {}) {
