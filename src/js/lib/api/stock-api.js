...
async function stockGet(pathWithQuery) {
    ...
    if (!result.ok)
        return { ok: false, error: result.error || "NO_SUCCESS_LOAD" };
...
export async function searchUnsplash(params) {
    ...
    if (params.orientation) {
        const o = params.orientation === "square" ? "squarish" : params.orientation;
...
export async function searchPexelsVideos(params) {
    ...
    if (params.query)
        search.set("query", params.query);
    if (params.orientation) {
        const o = params.orientation === "squarish" ? "square" : params.orientation;
        search.set("orientation", o);
    }
    search.set("page", String(params.page ?? 1));
    search.set("perPage", String(params.perPage ?? 30));
    const res = await stockGet(`${STOCK_PEXELS_VIDEOS_ENDPOINT}?${search.toString()}`);
    if (!res.ok)
        return { error: res.error };
    try {
        return { data: JSON.parse(res.text) };
...
export function stockDownloadUrl(opts) {
    ...
    if (opts.quality)
        search.set("quality", opts.quality);
    if (opts.resolution)
        search.set("resolution", opts.resolution);
        ...
        await downloadToFile(url, filePath, {
            ...
            onErrorBody: (status, body) => {
                ...
                try {
                    ...
                    throw Object.assign(new Error(parsed.error || `Download failed (${status})`), { status });
                }
                catch (e) {
