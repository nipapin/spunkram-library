...
import { fetchCepMarketStructure, resolvePackEntitlementContextForScan, } from "@/api/cep-market";
...
import { activePackStorageKey, currentPackHost, } from "@/lib/utils/pack-host";
import { loadInstalledPack, packInitErrorMessage, readInstallablePackages, } from "@/lib/utils/pack";
import { buildPackTree, collectAllContentSections, collectContentSections, filterContentSections, filterFavoriteSections, filterPackTree, filterUnlockedSections, findPackRootFor, findPackTreeNode, firstGroupIdIn, getFirstPackRoot, } from "@/lib/utils/pack-tree";
import { buildPackEntitlementContext, isPackEntitled, } from "@/lib/utils/pack-entitlement";
import { itemUnlockedForGalPlan, resolveGalAccountPlan, } from "@/lib/utils/gal-plan";
import { ensureGalItemOnDisk, ensureGalVirtualPackLayout, galItemLocallyCached, galOfflineAssetsPresent, resolveGalInstallRoot, syncGalAssetsFromManifest, } from "@/lib/utils/gal-effects-cache";
...
export function readActivePackPath(host = currentPackHost()) {
...
export function writeActivePackPath(packPath, host = currentPackHost()) {
    const key = hostActivePackKey(host);
    if (!key || !packPath)
        return;
    try {
...
function persistCategoryForPack(packPath, categoryId) {
    if (!packPath || !categoryId || categoryId === FAVORITES_CATEGORY_ID)
        return;
    try {
...
function resolveCategoryForPack(tree, packKey) {
...
function pickMarketPack(packages, host) {
    if (!packages.length)
        return null;
    if (!host)
        return packages[0] ?? null;
    return (packages.find((p) => (p.primary_type || "").toUpperCase() === host) ||
        packages[0] ||
        null);
}
export function usePackWorkspace() {
    const { signedIn, subscription, market, authReady, refreshMarket } = useAuth();
    const { showFavoritesOnly, showAvailableOnly, favoriteIds, setShowFavoritesOnly } = usePanelUI();
    ...
    const [assetsHost, setAssetsHost] = useState(null);
    const [hasInstalledPacks, setHasInstalledPacks] = useState(() => readInstallablePackages().length > 0);
    const [structureLoading, setStructureLoading] = useState(() => BRAND.id === "gal");
    const [galAssetsSync, setGalAssetsSync] = useState(null);
    ...
    const setCategory = useCallback((id) => {
        ...
        if (id)
            lastRealCategoryRef.current = id;
        setShowFavoritesOnly(false);
        setCategoryState(id);
    }, [setShowFavoritesOnly]);
    const hydrateLocalPack = useCallback(async (meta) => {
    ...
    const applyLocalTree = useCallback((loaded) => {
        ...
        if (nextCategory)
            lastRealCategoryRef.current = nextCategory;
    ...
    const runGalAssetsSync = useCallback((opts) => {
        if (BRAND.id !== "gal")
            return;
        const host = opts?.host ??
            ...
            currentPackHost() ??
            "PR";
        const packVersion = opts?.packVersion ?? galSyncMetaRef.current?.packVersion;
        ...
        void syncGalAssetsFromManifest({
            ...
            packEtag,
            onProgress: (p) => {
                if (syncGen !== galSyncGenRef.current)
                    return;
                if (p.phase === "done")
                    return;
                setGalAssetsSync(p);
    ...
    useEffect(() => {
        if (!packFilePath || !category || category === FAVORITES_CATEGORY_ID)
            return;
    ...
    useEffect(() => {
        ...
        if (category === FAVORITES_CATEGORY_ID) {
            const restore = lastRealCategoryRef.current || getFirstPackRoot(tree)?.id || "";
    ...
    useEffect(() => {
        ...
        void (async () => {
            if (isGal) {
                ...
                if (gen !== structureGenRef.current)
                    return;
                setStructureLoading(false);
                if (remote.data?.content) {
                    ...
                    setCategoryState((prev) => {
                        if (prev === FAVORITES_CATEGORY_ID)
                            return prev;
                        if (prev && findPackTreeNode(nextTree, prev))
                            return prev;
                ...
                if (structureSourceRef.current === "local")
                    return;
                setAssetsBaseUrl("");
                setAssetsHost(null);
                if (readInstallablePackages().length === 0) {
                    setTree([]);
                    setStructureSource(null);
                    setPackError(remote.error === "NO_PACK"
            ...
            if (readInstallablePackages().length === 0) {
                setTree([]);
                setStructureSource(null);
                setPackError(remote.error === "NO_STRUCTURE"
    ...
    const isEntitled = useMemo(() => {
        ...
        return (activePackMeta != null && isPackEntitled(activePackMeta, entitlementCtx));
    }, [signedIn, activePackMeta, entitlementCtx]);
    /** Entitled + (local zip assets OR Gal on-demand path). */
    const canApply = useMemo(() => isEntitled && (hasLocalAssets || galCanDownloadApply), [isEntitled, hasLocalAssets, galCanDownloadApply]);
    /**
     * Lock chip / gate: Gal per-item `plan[]` vs account plan; Spunkram pack-level.
     */
    const isLocked = useCallback((item) => {
        if (BRAND.id === "gal") {
            if (!signedIn || !galAccountPlan)
                return true;
    ...
    useEffect(() => {
        ...
        if (category && findPackTreeNode(sidebarTree, category))
            return;
    ...
    const activeRootId = useMemo(() => {
        ...
        if (!browseTree.length)
            return "";
        const root = findPackRootFor(browseTree, category) ?? getFirstPackRoot(browseTree);
        return root?.id ?? "";
    }, [
        tree,
        sidebarTree,
    ...
    const sections = useMemo(() => {
        if (!tree.length)
            return [];
        const browseTree = showAvailableOnly ? sidebarTree : tree;
        if (!browseTree.length && showAvailableOnly)
            return [];
        const hasQuery = query.trim().length > 0;
        let next;
        if (showFavoritesOnly || category === FAVORITES_CATEGORY_ID) {
            next = filterContentSections(filterFavoriteSections(allSections, favoriteIds), query);
        }
        else if (hasQuery) {
     ...
     */
    const prepareApply = useCallback(async (item) => {
        if (BRAND.id !== "gal") {
            if (!packFilePath) {
                return {
                    ok: false,
        ...
        const host = (assetsHost ?? currentPackHost() ?? "PR");
