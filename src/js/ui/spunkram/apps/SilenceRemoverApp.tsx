...
import { silenceRangesFromTokens, type SilenceToken } from "@/utils/silence-ranges";
import { normalize, transcribe, type TranscribeResult } from "@/utils/transcribe";
...
const tokensFromTranscription = (result: TranscribeResult): SilenceToken[] => {
...
export const SilenceRemoverApp = ({
  ...
  const showError = (err: unknown) => {
    ...
    if (!msg || isAbortLikeError(err)) return;
  ...
  const run = async (signal: AbortSignal) => {
    ...
    if (await isAfterEffectsAsync()) {
      const err = new Error(
        "Silence Remover cuts clips in Premiere Pro. Open the sequence there and try again.",
      );
    ...
    let source: string | undefined = exported.source;
    let dest: string | undefined = exported.dest;
    try {
      ...
      if (data && data.ok === false) {
        ...
        if (data.reason === "NO_ACTIVE_SEQUENCE" || data.reason === "PPRO_ONLY") {
          (err as Error & { soft?: boolean; reason?: string }).soft = true;
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      if (!isAbortLikeError(e)) {
        showError(e);
        if (!isSoftHostError(e)) reportSupportError("silence.remove", e);
      }
    } finally {
  ...
  return (
    <div className="silence-app">
      <div className="silence-app__body thin-scroll">
        <div className="card silence-app__card">
          <div className="silence-app__card-head">
            <p className="silence-app__card-title">
              <Scissors className="size-3.5" strokeWidth={2.25} />
              Silence Remover
            </p>
          </div>
          <p className="silence-app__lead">
            Transcribes the sequence In/Out, finds pauses between words, and cuts them with the
            razor on every unlocked track.
          </p>
          <div className="silence-app__sliders">
            <div className="field-row silence-app__slider-row">
              <span className="field-row__label">Pause</span>
              <input
                type="range"
                className="range"
                min={MIN_GAP_MIN}
                ...
                aria-label="Minimum pause to cut"
                style={rangeFillStyle(minGap, MIN_GAP_MIN, MIN_GAP_MAX)}
              />
              <span className="silence-app__value">{minGap.toFixed(1)}s</span>
            </div>
            <div className="field-row silence-app__slider-row">
              <span className="field-row__label">Pad</span>
              <input
                type="range"
                className="range"
                min={PAD_MIN}
                ...
                aria-label="Padding kept around each word"
                style={rangeFillStyle(pad, PAD_MIN, PAD_MAX)}
              />
              <span className="silence-app__value">{pad.toFixed(2)}s</span>
            </div>
          </div>
          {summary ? <p className="silence-app__summary">{summary}</p> : null}
        </div>
      </div>
      <div className="silence-app__footer">
        <button
          type="button"
        ...
        <p className="silence-app__hint">Set In and Out around the section you want tightened.</p>
      </div>
      <ProgressDialog
        progress={progress}
