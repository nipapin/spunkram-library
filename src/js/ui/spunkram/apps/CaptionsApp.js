...
import { ProgressDialog, CAPTIONS_PROGRESS_STEPS, LOAD_CAPTIONS_PROGRESS_STEPS, } from "../../../components/ProgressDialog";
...
import { captionsToChunks, clampTranscriptionToSpeechStart, grouping, moveWordToAdjacent, normalize, rebuildWords, splitIntoWords, transcribe, } from "../../../utils/transcribe";
...
const findCaptionIndex = (captions, t) => {
    if (!captions.length)
        return null;
    let bestIndex = 0;
...
export const CaptionsApp = ({ generationsLeft = 0, }) => {
    const { mode, lines, characters, fontSize, mogrtPath, aepPath, audioPresetPath, srcLang, translateTo, presets, selectedPresetId, selectPreset, ensureStyleDownloaded, applySelectedPresetToHost, syncSelectedPresetFontFromHost, ensureDefinitionLoaded, updateMode, updateLines, updateCharacters, } = useConfiguration();
    ...
    const [meta, setMeta] = useState({ type: "composition", offset: 0 });
    ...
    const captions = useMemo(() => (data ? grouping(data, { mode, lines, characters }, { customSegments }) : []), [data, mode, lines, characters, customSegments]);
    ...
    const persistCustomSegments = (next) => {
        customSegmentsRef.current = next;
        if (next)
            panelStore.setItem(CUSTOM_SEGMENTS_KEY, JSON.stringify(next));
        else
            panelStore.removeItem(CUSTOM_SEGMENTS_KEY);
        setCustomSegments(next);
    };
    const persistAppliedConfig = (next) => {
        if (next)
            panelStore.setItem(APPLIED_CONFIG_KEY, JSON.stringify(next));
        else
            panelStore.removeItem(APPLIED_CONFIG_KEY);
        setAppliedConfig(next);
    };
    // лениво инициализировать customSegments из текущего UI (для words/custom правок)
    const ensureCustomSegments = () => {
        if (customSegmentsRef.current)
            return customSegmentsRef.current;
    ...
    const pushHostResegment = (opts) => {
        ...
        const custom = opts && "customSegments" in opts ? opts.customSegments : customSegmentsRef.current;
        const resegmented = grouping(source, { mode: modeRef.current, lines: linesRef.current, characters: charactersRef.current }, { customSegments: custom });
        ...
        return sdkData(hostSdk().resegmentCaptions({
    ...
    const applySegmentEdit = (nextChunks, caption) => {
    ...
    const handleLoad = async () => {
        ...
        try {
            const loaded = (await sdkData(hostSdk().loadCaptionsFromTimeline()));
            if (!loaded?.segments?.length)
                return;
            const wordChunks = loaded.segments.map((s) => ({
                text: s.text,
                timestamp: s.timestamp,
            }));
            const nextMode = typeof loaded.segmentType === "number"
                ? groupingModeFromSegmentType(loaded.segmentType)
                : modeRef.current;
            const nextLines = typeof loaded.lineCount === "number"
                ? Math.max(1, Math.round(loaded.lineCount))
                : linesRef.current;
            const nextChars = typeof loaded.charsPerLine === "number"
                ? Math.max(1, Math.round(loaded.charsPerLine))
                : charactersRef.current;
            updateMode(nextMode);
            if (typeof loaded.lineCount === "number")
                updateLines(nextLines);
            if (typeof loaded.charsPerLine === "number")
                updateCharacters(nextChars);
            const dataFromTimeline = normalize({
                chunk: { chunks: [] },
            ...
            const hostRef = typeof loaded.compId === "number"
    ...
    const runTranscription = async (signal) => {
        ...
        if (selected) {
            try {
                ...
                if (paths?.mogrt)
                    activeMogrtPath = paths.mogrt;
                if (paths?.aep)
                    activeAepPath = paths.aep;
            }
            catch (e) {
        ...
        if (!isAfterEffects() && !activeMogrtPath) {
            throw new Error("This style has no .mogrt file for Premiere Pro. Select a style with Premiere support and try again.");
        }
        ...
        if (isAfterEffects() && selected && !activeAepPath && !activeMogrtPath) {
            throw new Error("This style has no .aep file for After Effects. Select a style with After Effects support and try again.");
        }
        ...
        try {
            ...
            throwIfCancelled(signal);
            if (!created.ok) {
                throw new Error(created.error || "Could not create captions on the timeline. Select a style and try again.");
            }
            ...
            const hostRef = createResult && typeof createResult.trackIndex === "number"
                ? {
            ...
            const finalMeta = {
                ...
                durationSeconds: placedDuration > 0 ? placedDuration : nextMeta.durationSeconds,
                ...(hostRef ? { hostRef } : {}),
            ...
            if (hostRef) {
                ...
                const saveArgs = "compId" in hostRef
    ...
    const pushLiveEdit = (source) => {
        ...
        const resegmented = grouping(source, { mode: modeRef.current, lines: linesRef.current, characters: charactersRef.current }, { customSegments: customSegmentsRef.current });
        ...
        hostSdk()
            .updateCaptionText({
            trackIndex: premiere ? hostRef.trackIndex : undefined,
            clipIndex: premiere ? 0 : undefined,
            sequenceId: premiere && "sequenceId" in hostRef ? hostRef.sequenceId : undefined,
            compId: !premiere ? hostRef.compId : undefined,
    ...
    const applyWordTextEdit = (source, caption, text) => {
        const wordChunks = source.words.chunks ?? [];
        const indices = caption.edit.target === "words"
    ...
    const handleSaveCaption = useCallback((caption, index, text) => {
        const source = dataRef.current;
        if (!source)
            return;
        ...
        if (caption.edit.target === "sentence") {
            ...
            const sentenceChunks = (source.chunk.chunks ?? []).map((c, i) => i === idx ? { ...c, text: trimmed } : c);
            const next = applyWordTextEdit({ ...source, chunk: { ...source.chunk, chunks: sentenceChunks } }, caption, trimmed);
            persist(next);
            pushLiveEdit(next);
            return;
        }
    ...
    const handleSplit = useCallback((caption, wordPos) => {
        const source = dataRef.current;
        if (!source || caption.edit.target !== "sentence" || !caption.words)
            return;
    ...
    const handleMerge = useCallback((caption, dir) => {
        const source = dataRef.current;
        if (!source || caption.edit.target !== "sentence")
            return;
    ...
    const handleMoveWord = useCallback((caption, uiIndex, dir) => {
    ...
    const handleSplitIntoWords = useCallback((caption, uiIndex) => {
    ...
    const handleSeek = (caption, index) => {
    ...
    const syncToTime = (t) => {
        ...
        if (bestIndex === lastSyncedIndexRef.current)
            return;
    ...
    useEffect(() => {
        ...
        const tick = async () => {
            if (!captions.length || screen !== "editor")
                return;
            if (document.hidden || isTypingInField())
                return;
            try {
                const current = await sdkData(hostSdk().getCurrentTime());
                if (current == null || typeof current !== "object")
                    return;
                const { time, compId } = current;
                // AE: не скроллим чужие композиции
                if (isAfterEffects()) {
                    const sourceId = metaRef.current.sourceCompId;
                    const captionsCompId = metaRef.current.hostRef && "compId" in metaRef.current.hostRef
                        ? metaRef.current.hostRef.compId
                        : undefined;
                    if (sourceId != null || captionsCompId != null) {
                        const allowed = (sourceId != null && compId === sourceId) ||
                            (captionsCompId != null && compId === captionsCompId);
                        if (!allowed)
                            return;
                    }
                }
                if (time === lastSyncedTimeRef.current)
                    return;
    ...
    useEffect(() => {
        ...
        if (!hostRef || !source)
            return;
        const canSave = ("compId" in hostRef && isAfterEffects()) ||
    ...
    return (_jsxs("div", { className: "app-shell", children: [_jsx(ProgressDialog, { progress: progress, onCancel: handleCancelDescribe, title: "Generating captions", steps: CAPTIONS_PROGRESS_STEPS }), _jsx(ProgressDialog, { progress: loadProgress, title: "Loading captions", steps: LOAD_CAPTIONS_PROGRESS_STEPS }), _jsx(CaptionsTab, { captions: captions, meta: meta, progress: progress, loadingCaptions: !!loadProgress, sentenceCount: data?.chunk.chunks?.length ?? 0, fontSize: fontSize, highlightIndex: highlightIndex, screen: screen, onLoad: handleLoad, onDescribe: handleDescribe, transcribeLabel: withGenerationCostLabel("Transcribe", generationCost), onBack: handleBack, onSaveCaption: handleSaveCaption, onSeek: handleSeek, onSplit: handleSplit, onMerge: handleMerge, onMoveWord: handleMoveWord, onSplitWords: handleSplitIntoWords, onUpdateResegment: handleUpdateResegment, appliedResegmentConfig: appliedConfig, resegmenting: resegmenting })] }));
};
