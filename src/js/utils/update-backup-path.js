...
export function isUpdateBackupName(name) {
    if (!name)
        return false;
    if (name === NATIVE_BACKUP_DIR)
        return true;
    if (name.includes(UPDATE_OLD_SUFFIX))
        return true;
    if (name.endsWith(PENDING_SUFFIX))
        return true;
    if (STACKED_OLD_RE.test(name))
        return true;
    if (TRAILING_OLD_RE.test(name))
        return true;
    return false;
}
/** Strip leftover updater suffixes so the live basename stays short. */
export function liveNativeBaseName(name) {
    let out = name;
...
export function allocateNativeBackupPath(target, opts) {
