...
export function hashedAssetRedirectStub(targetHtmlFromThisFile) {
    const dest = targetHtmlFromThisFile.replace(/\\/g, "/");
    return `(function(){try{window.location.replace("${dest}?_cep_upd="+Date.now());}catch(e){}})();`;
}
export function rewriteHtmlAssetDepth(html, fromDepth, toDepth) {
...
function currentHashedMains(assetsDir, ext) {
 ...
 */
export function writeLegacyCepEntries(outDir, brandId) {
    const written = [];
