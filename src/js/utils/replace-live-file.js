import { allocateNativeBackupPath, isUpdateBackupName, NATIVE_BACKUP_DIR, PENDING_SUFFIX, } from "./update-backup-path";
...
export function isBusyError(err) {
    ...
    if (Number.isFinite(errno) && BUSY_ERRNO.has(errno))
        return true;
    const msg = typeof o.message === "string" ? o.message : String(err);
    return /EPERM|EBUSY|EACCES|operation not permitted|not permitted|resource busy|being used by another process|locked/i.test(msg);
}
...
function unlinkBestEffort(io, target) {
    try {
...
function allocateBackup(io, target) {
...
function retryBusy(fn, attempts = 3) {
    let last;
...
function relToExt(io, extRoot, abs) {
 ...
 */
export function copyFileOverwrite(io, from, to, extRoot) {
    if (isUpdateBackupName(io.path.basename(to)))
        return null;
    try {
        ...
        if (!isBusyError(err) || !io.fs.existsSync(to))
            throw err;
    }
    ...
    try {
        ...
        try {
            ...
            if (!isBusyError(copyErr))
                throw copyErr;
        }
    }
    catch (renameErr) {
        if (!isBusyError(renameErr))
            throw renameErr;
    }
...
export function cleanupUpdateBackups(io, root) {
        ...
        try {
            ...
            if (st.isDirectory()) {
                if (name === NATIVE_BACKUP_DIR) {
                    ...
                    try {
                        if (io.fs.readdirSync(full).length === 0)
                            io.fs.rmdirSync?.(full);
                    }
                    catch {
                        /* still locked */
                    }
...
function collectPendingRels(io, extRoot, dir, out) {
    if (!io.fs.existsSync(dir))
        return;
    let entries;
    try {
...
export function promotePendingUpdates(io, extRoot, markerFiles = []) {
    const candidates = new Set(markerFiles);
...
export function pendingNativesOnly(paths) {
