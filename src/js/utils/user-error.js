...
function collectErrorBits(err) {
    ...
    const visit = (value, depth) => {
        ...
        if (typeof o.code === "string" && o.code)
            codes.push(o.code);
        if (typeof o.errno === "string" && o.errno)
            codes.push(o.errno);
        if (typeof o.message === "string" && o.message)
            texts.push(o.message);
        if (typeof o.syscall === "string" && o.syscall)
            texts.push(o.syscall);
...
function looksLikeInternalError(text) {
    if (!text)
        return false;
    return (/\bE(?:TIMEDOUT|SOCKETTIMEDOUT|CONNREFUSED|CONNRESET|CONNABORTED|NOTFOUND|PIPE|PROTO|HOSTUNREACH|NETUNREACH|NETDOWN|AI_AGAIN|ACCES|PERM|NOENT|NOSPC|ADDRNOTAVAIL|NOTCONN|HOSTDOWN)\b/.test(text) ||
...
function networkish(text, codes) {
    if (codes.some((c) => TIMEOUT_CODES.has(c)))
        return "timeout";
    if (codes.some((c) => SSL_CODES.has(c)))
        return "ssl";
    if (codes.some((c) => NETWORK_CODES.has(c)))
        return "network";
    if (/\bETIMEDOUT\b|\bESOCKETTIMEDOUT\b|timed\s*out|timeout/i.test(text))
        return "timeout";
    if (/self[- ]signed|certificate|UNABLE_TO_|CERT_|ERR_TLS_|ERR_SSL_/i.test(text))
        return "ssl";
    if (/failed to fetch|networkerror|net::err|no_connection|enotfound|econnreset|econnrefused|eai_again|socket hang up|getaddrinfo|network is unreachable/i.test(text)) {
        return "network";
    }
    return null;
}
function mapTechnicalMessage(text, codes) {
    const kind = networkish(text, codes);
    if (kind === "timeout")
        return TIMEOUT_ERROR;
    if (kind === "ssl")
        return "Secure connection failed. Check your network or VPN and try again.";
    if (kind === "network")
    ...
    if (looksLikeInternalError(text))
...
export function isAbortLikeError(err) {
    ...
    if (bits.codes.some((c) => ABORT_CODES.has(c.toUpperCase())))
        return true;
    return /\b(?:download |installation )?cancell?ed\b/i.test(bits.text);
}
/** Soft host failures the user can fix without support. */
export function isSoftHostError(err) {
    if (err && typeof err === "object" && err.soft)
        return true;
    const msg = err instanceof Error ? err.message : String(err ?? "");
    const reason = err && typeof err === "object" && typeof err.reason === "string"
        ? err.reason
        : "";
    ...
    return (/open a sequence first/i.test(msg) ||
...
export function friendlyErrorMessage(err) {
    ...
    if (typeof err === "string") {
        if (err === "NO_SUCCESS_LOAD")
            return "Unable to reach Motionflow. Try again in a moment.";
    ...
    if (!msg || msg === "null" || msg === "undefined")
        return GENERIC_ERROR;
    if (/^cancelled$/i.test(msg) || /^download cancelled$/i.test(msg))
        return msg;
