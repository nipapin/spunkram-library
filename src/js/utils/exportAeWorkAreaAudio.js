 ...
 */
async function readWorkAreaAudioInfo() {
    const raw = String(await evalES(`(function(){
    ...
    if (!(durationSeconds > 0)) {
        throw new Error(`Could not read the Work Area (host answered "${raw || "nothing"}"). Reload the panel and try again.`);
    }
    if (parts[3] !== "1") {
...
function queueAudioRender(esOutputPath, offset, durationSeconds) {
    void evalES(`(function(){
      try {
        var comp = app.project.activeItem;
...
async function isQueuedForOutput(fileMarker) {
    const raw = String(await evalES(`(function(){
...
function findRenderedFile(dir, fileMarker) {
    try {
        for (const name of fs.readdirSync(dir)) {
            if (name.indexOf(fileMarker) !== 0)
                continue;
            if (/\.mp3$/i.test(name))
                continue; // our own transcode target
 ...
 */
async function waitForRenderedAudio(dir, fileMarker, timeoutMs, stableMs = 1000) {
 ...
 */
function removeQueuedItems(fileMarker) {
    void evalES(`(function(){
      try {
 ...
 */
export async function exportAeWorkAreaAudio() {
    ...
    if (!queued) {
        throw new Error("Could not add the composition to the render queue. Reload the panel and try again.");
    }
    startRender();
    let renderedFile;
    try {
