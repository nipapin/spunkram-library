...
const unsetProbe = (error) => ({
    durationSeconds: 0,
    cost: 1,
    error,
});
const durationFromPayload = (raw) => {
    let data = raw;
    for (let i = 0; i < 3; i++) {
 ...
 */
export const useWorkRangeCost = (enabled = true) => {
    const [probe, setProbe] = useState({ durationSeconds: 0, cost: 1 });
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            if (durationSeconds > 1e9)
                durationSeconds = durationSeconds / 254016000000;
            ...
            const next = unsetProbe(e instanceof Error ? e.message : "Could not read timeline range");
            setProbe(next);
            return next;
        }
    }, [enabled]);
