...
function loadGenerationsState(limit) {
    ...
    try {
        ...
        if (parsed.monthKey !== fallback.monthKey)
            return fallback;
...
function saveGenerationsState(state) {
    try {
...
export function useGenerationsBalance() {
    ...
    const refresh = useCallback(async () => {
        ...
        if (!status?.authenticated)
            return;
        const nextMonthly = typeof status.subscription_generations_left === "number"
            ? status.subscription_generations_left
            : typeof status.remaining === "number"
                ? status.remaining
                : null;
        const nextExtra = typeof status.extra_generations_left === "number"
            ? status.extra_generations_left
            : null;
        if (nextMonthly !== null)
            setMonthly(Math.max(0, nextMonthly));
        if (nextExtra !== null)
            setExtra(Math.max(0, nextExtra));
    }, [subscribed]);
    useEffect(() => {
