...
const BAR_COUNT = 120;
const FLAT_PEAKS = new Array(BAR_COUNT).fill(0);
const CONCURRENCY = 2;
const queue = [];
let running = 0;
function drain() {
...
function enqueue(run) {
    const item = { run, cancelled: false };
    queue.push(item);
    drain();
    return item;
}
...
async function loadAudioArrayBuffer(audioUrl) {
...
async function fetchPeaks(audioUrl) {
...
function claimPlayback(track) {
    if (activeTrack && activeTrack !== track)
        activeTrack.pause();
    activeTrack = track;
}
function releasePlayback(track) {
...
function drawWaveform(canvas, peaks, progress) {
    ...
    function buildPath(startIdx, endIdx) {
        ctx.beginPath();
 ...
 */
export function WaveformPlayer({ audioUrl, loading = false, eagerLoad = true, className, buttonClassName, waveformClassName, leadingSlot, trailingSlot, timeClassName, }) {
    ...
    useEffect(() => {
        ...
        const obs = new IntersectionObserver(([entry]) => {
            if (!entry.isIntersecting || queueItem || cancelled)
                return;
            obs.disconnect();
            start();
        }, { rootMargin: "120px" });
    ...
    const handleRef = useRef({ pause: pauseThis });
    useEffect(() => {
    ...
    return (_jsxs("div", { ref: rowRef, className: cn("flex min-w-0 items-center gap-2", className), children: [_jsx("button", { type: "button", onClick: togglePlay, disabled: disabled, className: cn("flex size-8 shrink-0 items-center justify-center rounded-full border border-primary/50 bg-primary/15 text-primary transition-colors", "hover:bg-primary/25 disabled:cursor-not-allowed disabled:opacity-50", buttonClassName), "aria-label": showSpinner ? "Loading" : isPlaying ? "Pause" : "Play", children: showSpinner ? (_jsx(Loader2, { className: "size-3.5 animate-spin" })) : isPlaying ? (_jsx(Pause, { className: "size-3.5" })) : (_jsx(Play, { className: "size-3.5 fill-current" })) }), leadingSlot, _jsx("div", { className: cn("relative h-9 min-w-0 flex-1", waveformClassName), children: _jsx("canvas", { ref: canvasRef, onClick: handleWaveformClick, className: "h-full w-full cursor-pointer text-foreground" }) }), _jsx("span", { className: cn("w-9 shrink-0 text-right text-[10px] tabular-nums text-muted-foreground", timeClassName), children: duration > 0
                    ? isPlaying
