...
function PathBrowse({ value, disabled, onBrowse }: { value: string; disabled?: boolean; onBrowse: () => void }) {
  return (
        ...
        <div className="mt-3 flex gap-2">
          <button
            ...
            className={cn(
              ...
              destructive ? "border border-destructive/50 bg-destructive/20 text-destructive" : ACCENT_PILL,
            )}
          >
            {confirmLabel}
          </button>
...
export function SettingsPanel({ onBack }: { onBack: () => void }) {
  ...
  function browseAssets() {
    selectFolder(prefs.customStockLocation || "", "Select assets folder", (folder) =>
      patch({
    ...
    <div className="relative flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2.5">
        <section className="mb-3 glass-card rounded-[20px] p-3">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">File System</h3>

          <div className="mb-1 text-xs text-foreground">Packages path</div>
          <p className="mb-1 text-[10px] text-muted-foreground">
            Packs install under AE/ or PR/ inside this folder. Existing packs there are imported automatically when you pick or change
            this path.
          </p>
          <PathBrowse value={prefs.absCustomAbsolutePath || ""} onBrowse={browsePackages} />
          {packagesScanMsg ? <p className="mt-1.5 text-[10px] text-primary">{packagesScanMsg}</p> : null}
        ...
        {isAdmin && (
          <section className="mb-3 glass-card rounded-[20px] p-3 ring-1 ring-inset ring-primary/20">
            ...
            <p className="mb-2 text-[10px] text-muted-foreground">
              All uploaded ZXPs from R2. Current: <span className="text-foreground">v{EXTENSION_VERSION}</span>
              {pointerBeta ? (
                <>
                  {" "}
            ...
            {versionsLoading ? (
              ...
              <>
                ...
                >
                  {installBusy ? <Loader2 className="size-3.5 animate-spin" /> : <ArrowUpCircle className="size-3.5" />}
                  {installBusy
        ...
        <section className="glass-card rounded-[20px] p-3">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Restoring An Extension</h3>
