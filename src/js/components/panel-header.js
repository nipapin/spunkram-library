...
export function PanelHeader({ active, onSelect, onOpenAccount, onOpenSettings, editingDisabled = false, }) {
    ...
    useLayoutEffect(() => {
        ...
        if (!header)
            return;
        const update = () => {
            if (header.dataset.phMeasuring === "1")
                return;
            ...
            if (keepLabels)
                header.classList.add("panel-header--labels");
    ...
    return (_jsxs("header", { ref: headerRef, className: showLabels ? "panel-header panel-header--labels" : "panel-header", children: [_jsx("button", { type: "button", className: "panel-header__logo", "aria-label": "Settings", title: "Extension settings", onClick: onOpenSettings, children: _jsx("img", { src: logo, alt: `${BRAND.authorName} logo`, width: 38, height: 38 }) }), _jsxs("nav", { ref: navRef, className: "panel-header__nav", children: [_jsx("span", { className: "panel-header__thumb" +
                            (thumb.visible ? " panel-header__thumb--on" : "") +
                            (animateThumb ? " panel-header__thumb--animate" : ""), style: {
                            transform: `translateX(${thumb.left}px)`,
                            width: thumb.width,
                        }, "aria-hidden": true }), NAV_ITEMS.map((item, index) => {
                        const Icon = item.icon;
                        const isActive = active === item.id;
                        const isDisabled = item.id === "editing" && editingDisabled;
                        return (_jsxs("button", { type: "button", ref: (node) => {
                                tabRefs.current[index] = node;
                            }, onClick: () => {
                                if (isDisabled)
                                    return;
                                onSelect(item.id);
                            }, disabled: isDisabled, "aria-label": item.label, title: isDisabled
                                ? "Install a pack from Market to enable Editing"
                                : item.label, "aria-pressed": isActive, className: isActive
                                ? "panel-header__tab panel-header__tab--active"
                                : isDisabled
                                    ? "panel-header__tab panel-header__tab--disabled"
                                    : "panel-header__tab", children: [_jsx(Icon, { className: "size-3.5", strokeWidth: 2.25 }), _jsx("span", { className: "panel-header__tab-label", children: item.label })] }, item.id));
                    })] }), _jsxs("button", { type: "button", className: active === "market"
                    ? "panel-header__market panel-header__market--active"
                    : "panel-header__market", onClick: () => {
                    clearUnread();
                    onSelect("market");
                }, "aria-pressed": active === "market", "aria-label": "Market", title: "Market", children: [activeCount > 0 ? (_jsx(Loader2, { className: "size-3.5 animate-spin", strokeWidth: 2.25 })) : (_jsx(ShoppingBag, { className: "size-3.5", strokeWidth: 2.25 })), _jsx("span", { className: "panel-header__tab-label", children: "Market" }), unreadMarketCount > 0 && (_jsx("span", { className: "panel-header__badge", children: unreadMarketCount > 9 ? "9+" : unreadMarketCount }))] }), _jsxs("button", { type: "button", className: active === "account"
                    ? "panel-header__account panel-header__account--active"
                    : "panel-header__account", "aria-label": "Account", onClick: onOpenAccount, children: [_jsx(User, { className: "size-4", strokeWidth: 2.25 }), signedIn && (_jsx("span", { className: subscription.subscribed
