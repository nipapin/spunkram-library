...
const previewByVoiceId = new Map();
const previewListeners = new Set();
let previewPreload = null;
...
async function fetchVoiceoverCatalogUncached() {
    ...
    if (result.ok) {
        try {
            ...
            const languages = Array.isArray(parsed.languages) && parsed.languages.length > 0
                ? parsed.languages
                : FALLBACK_LANGUAGES;
            if (voices.length > 0)
...
export async function fetchVoiceoverCatalog() {
    if (catalogCache?.voices.length)
        return catalogCache;
    if (!catalogPending) {
...
export async function fetchVoiceoverVoices() {
...
function voiceoverDir(subdir) {
    try {
        const roaming = typeof os?.platform === "function" && os.platform() === "darwin"
...
const previewBlobByVoiceId = new Map();
function mimeFromPreviewPath(filePath) {
...
function localAudioToBlobUrl(filePath) {
...
function rememberPreview(voiceId, filePath) {
    if (previewByVoiceId.get(voiceId) === filePath)
        return;
    previewByVoiceId.set(voiceId, filePath);
    emitPreviewUpdate();
}
async function ensureLocalPreview(voice) {
    const url = voice.preview_url?.trim();
    if (!url)
        return;
    const existing = previewByVoiceId.get(voice.id);
    if (existing && isValidAudioFile(existing))
        return;
 ...
 */
export function preloadVoiceoverPreviews() {
    if (previewPreload)
...
export async function downloadVoiceoverFile(audioUrl, fileName = `${BRAND.id}-voiceover.wav`) {
    try {
