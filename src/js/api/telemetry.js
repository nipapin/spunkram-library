import { TELEMETRY_ACTIVE_PACKS_ENDPOINT, TELEMETRY_INSTALLS_ENDPOINT, TELEMETRY_SESSION_ENDPOINT, } from "./config";
...
let reportedForUserId = null;
/** Dedup consecutive identical active-pack snapshots. */
let lastActivePacksKey = null;
 ...
 */
export async function reportClientSession() {
    const user = getUserIdentity();
    const token = user?.token;
    const userId = user?.id || null;
    if (!token || !userId)
        return;
    if (reportedForUserId === userId)
        return;
 ...
 */
export async function reportInstalledPacks() {
    const user = getUserIdentity();
    const token = user?.token;
    if (!token)
        return;
 ...
 */
export async function reportActivePacks(packIds) {
    const user = getUserIdentity();
    const token = user?.token;
    if (!token)
        return;
...
export function resetClientSessionTelemetryFlag() {
