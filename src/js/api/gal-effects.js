...
export const CEP_GAL_EFFECTS_MANIFEST_ENDPOINT = "/api/cep/gal/effects/assets-manifest";
 ...
 */
export async function fetchGalEffects(host = "PR") {
    ...
    const url = apiUrl(`${CEP_GAL_EFFECTS_ENDPOINT}?host=${encodeURIComponent(host)}`);
    ...
    try {
        ...
        const assetsBase = (raw.assets_base_url || "").trim() ||
        ...
        if (data.etag)
            cache.set(key, { etag: data.etag, data });
        return { data };
    }
    catch {
 ...
 */
export async function fetchGalAssetsManifest(host = "PR") {
    const token = getSessionToken();
    if (!token)
        return { error: "UNAUTHORIZED" };
    const key = host;
    ...
    if (cached?.etag)
        headers["If-None-Match"] = cached.etag;
    const url = apiUrl(`${CEP_GAL_EFFECTS_MANIFEST_ENDPOINT}?host=${encodeURIComponent(host)}`);
    const result = await cepHttpRequest(url, { method: "GET", headers });
    if (result.status === 304 && cached) {
        return { data: cached.data, notModified: true };
    }
    ...
    try {
        ...
        if (data.etag)
            manifestCache.set(key, { etag: data.etag, data });
        return { data };
    }
    catch {
 ...
 */
export async function fetchGalEffectsFileLink(host, relPath) {
    const token = getSessionToken();
    if (!token)
        return { error: "UNAUTHORIZED" };
    ...
    const url = apiUrl(`${CEP_GAL_EFFECTS_FILE_ENDPOINT}?host=${encodeURIComponent(host)}&path=${encodeURIComponent(path)}`);
...
export function galEffectsAssetUrl(assetsBaseUrl, segments, ext, host = "PR") {
