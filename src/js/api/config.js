...
export const UPDATE_ENDPOINT = "/api/cep/update";
/** Admin — full list of uploaded ZXP versions */
export const UPDATE_VERSIONS_ENDPOINT = "/api/cep/update/versions";
/** POST — CEP error observer → Telegram support topic (optional Bearer). */
export const SUPPORT_ENDPOINT = "/api/cep/support/report";
/** POST — host app version + OS after sign-in (Bearer required). */
export const TELEMETRY_SESSION_ENDPOINT = "/api/cep/telemetry/session";
/** POST — full snapshot of installed market packs on this device. */
export const TELEMETRY_INSTALLS_ENDPOINT = "/api/cep/telemetry/installs";
/** POST — packs currently open / focused (full snapshot). */
export const TELEMETRY_ACTIVE_PACKS_ENDPOINT = "/api/cep/telemetry/active-packs";
/**
...
export const apiUrl = (path) => path.startsWith("http") ? path : `${API_BASE}${path.startsWith("/") ? path : `/${path}`}`;
