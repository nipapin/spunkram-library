...
const SYSTEM_INIT_NAMES = new Set([
    ...CEP_WRITTEN_SYSTEM_NAMES,
    CAPTION_SYSTEM.rawData,
    "Captions_Data",
    "Captions_Raw_Data",
]);
const lastNameSegment = (name) => {
...
export const isChunkOrSystemInitName = (name) => {
    const n = String(name || "").trim();
    if (!n)
        return false;
    if (SYSTEM_INIT_NAMES.has(n))
        return true;
    const last = lastNameSegment(n);
    if (SYSTEM_INIT_NAMES.has(last))
        return true;
    if (n.startsWith("captions_batch_") || last.startsWith("captions_batch_"))
        return true;
    for (let i = 0; i < CAPTION_BATCH_NAMES.length; i++) {
        if (n === CAPTION_BATCH_NAMES[i] || last === CAPTION_BATCH_NAMES[i])
            return true;
    }
    return false;
};
const cloneJson = (value) => JSON.parse(JSON.stringify(value));
const controlValueToInitJson = (value) => {
...
const sanitizeNode = (node) => {
    ...
    if (Array.isArray(rec.controls)) {
        for (let i = 0; i < rec.controls.length; i++)
            sanitizeNode(rec.controls[i]);
    }
};
const sanitizeControlsTree = (doc) => {
    if (Array.isArray(doc.ui)) {
        for (let i = 0; i < doc.ui.length; i++)
            sanitizeNode(doc.ui[i]);
    }
    if (Array.isArray(doc.groups)) {
        for (let i = 0; i < doc.groups.length; i++)
            sanitizeNode(doc.groups[i]);
    }
    if (Array.isArray(doc.controls)) {
        for (let i = 0; i < doc.controls.length; i++)
            sanitizeNode(doc.controls[i]);
    }
};
/**
 ...
 */
export const mergeInitWithValues = (definition, values) => {
    const source = definition.init?.length
        ? definition.init
        : definition.controlsDocument?.init ?? [];
    if (!source.length) {
        const fromLeaves = [];
        for (const c of definition.clientControls ?? []) {
            if (isGroup(c))
                continue;
            const name = uiName(c);
            if (!name || isChunkOrSystemInitName(name))
                continue;
            if (values[c.id] === undefined && c.value === undefined)
                continue;
            fromLeaves.push({
                name,
                value: controlValueToInitJson((values[c.id] ?? c.value)),
...
export const buildUserControlsDocument = (parent, values) => {
    const doc = parent.controlsDocument
    ...
    const withParentInit = {
        ...parent,
        init: Array.isArray(doc.init) ? doc.init : parent.init,
    };
    doc.init = mergeInitWithValues(withParentInit, values);
    sanitizeControlsTree(doc);
    return doc;
};
export const definitionCacheKey = (preset) => preset.source === "user" ? preset.id : preset.styleId;
